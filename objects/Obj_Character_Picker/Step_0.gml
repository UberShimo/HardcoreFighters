...
else if(gamepad_button_check_pressed(controller_index, gp_padu) && !position_meeting(x+move_x_distance/2, y-move_y_distance, Parent_Collision)){
	y -= move_y_distance;
	x += move_x_distance/2;
}
else if(gamepad_button_check_pressed(controller_index, gp_padd) && !position_meeting(x-move_x_distance/2, y+move_y_distance, Parent_Collision)){
	y += move_y_distance;
	x -= move_x_distance/2;
}
